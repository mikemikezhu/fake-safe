...
# Load classifier
try:
    classifier = load_model('model/classifier_face_pain_rgb.h5')
except ImportError:
...
try:
    encoder_generator = load_model(
        'model/one_layer_face_mnist_rgb_encoder_generator.h5')
    decoder_generator = load_model(
        'model/one_layer_face_mnist_rgb_decoder_generator.h5')
except ImportError:
    print('Unable to load encoder and decoder. Please fun "one_layer_mnist_rgb" script first')
    sys.exit()

""" Face RGB -> MNIST -> Face RGB """
...
original_name = 'Original - Face RGB -> MNIST -> Face RGB'
...
encoded_name = 'Encoded - Face RGB -> MNIST -> Face RGB'
...
decoded_name = 'Decoded - Face RGB -> MNIST -> Face RGB'
decoded_images = (decoded_images_scaled + 1) / 2 * 255
decoded_images = decoded_images.astype(int)
