...
from generator_models import TextTrainModelCreator
...
from trainers import EncoderTrainer, DecoderTrainer, TextDecoderTrainer
...
text_trainer_model_creator = TextTrainModelCreator(
    vocabulary_size, max_sequence_length)
text_trainer_model, encoder_embedding, encoder_lstm, decoder_embedding, decoder_lstm, decoder_dense = text_trainer_model_creator.create_model()

decoder_text_trainer = TextDecoderTrainer(text_trainer_model,
...
# text -> state
text_encoder_creator = TextEncoderGeneratorModelCreator(encoder_embedding,
                                                        encoder_lstm,
                                                        max_sequence_length)
...
# state -> text
text_decoder_creator = TextDecoderGeneratorModelCreator(decoder_embedding,
                                                        decoder_lstm,
                                                        decoder_dense)
...
for current_round in range(constants.TOTAL_TRAINING_ROUND):
    ...
    states_arr = np.asarray(hidden_states)

    # state -> image
    encoder_trainer.train(states_arr, fashion_image_train_scaled)

    # state -> image -> state
    decoder_state_trainer.train(states_arr, None)
