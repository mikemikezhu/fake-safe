...
class TextDecoderTrainer(AbstractModelTrainer):
    ...
    def train(self, input_data, exp_output_data):
        ...
        x_train = np.asarray(self.input_data)
        y_train = self.__create_one_hot_decoder_targets(x_train)
        history = self.trainer_model.fit([x_train, x_train],
                                         y_train,
    ...
    def __create_one_hot_decoder_targets(self, sequences):
        # Create decoder targets in one hot format
        # Reference:
        ...
        # The shape of decoder targets will be:
        # (batch size, sequence length, vocabulary size)
        # i: the index of the sentence in batch
        # j: the index of the word in sentence
        # word: the index of the word in word embedding
        for i, sequence in enumerate(sequences):
            for j, word in enumerate(sequence):
                decoder_targets[i, j, word] = 1
